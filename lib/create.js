...
function create (options) {
  ...
  validateNew(name, options.force);
 ...
 * @param      {Boolean}  force     Whether to override existing
 */
function validateNew(taskName = '', force = false) {
  ...
  // Validate existing
  if ( fs.existsSync(projectRoot(`gulp/${taskName}.js`)) && !force ) {
    print(`Task ${taskName.bold} is already present! You can either just update it, or force to override it using ${'--force'.bold} option.`, new Error());
  }
