...
/**
 * Gets the list of all installed tasks
 *
 * @function      All 
...
const All = () => {
  ...
  if ( !existsSync(projectRoot(tasksDir)) ) {
    print(`Enable to load tasks from directory ${tasksDir.bold}. Please make sure that it is existing and spelled correctly.`, 'error');
    process.exit();
  }
  ...
  filesSync(projectRoot(tasksDir), (basedir, filename, stat) => {
    ...
    try {
      ...
      print(`Unable to load task file ${filename.bold} due to an error:`, err);
    }
