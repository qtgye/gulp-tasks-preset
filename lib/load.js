...
module.exports = ( ...taskNames ) => {
  ...
  // If taskNames is empty, load all tasks
  if ( !taskNames || !taskNames.length) {
    
    // If tasksDir is empty, use empty taskNames
    if ( !existsSync(config('tasksDir'))) {
      taskNames = [];
    }
    else {
      filesSync(projectRoot(config('tasksDir')), (basedir, filename, stat) => {
        taskNames.push(filename.replace(/\..*$/, ''));
      });
    }
  }
  ...
  taskNames.forEach( taskName => {
    ...
    if ( !('task' in taskObject)) {
      return print(`The task ${taskName.bold} has no callable ${'task'.bold} function. Please update the task file.`, 'warning');
    }
  ...
  // Load "default" task
  let defaultTask = _default.get();
  defaultTask.dependencies = defaultTask.dependencies || [];
  gulp.task('default', done => {
    run.fromObject(defaultTask).then(done);
  });
  ...
  gulp.task('tasks', done => {
    run.fromObject(require(packageRoot('lib/tasks'))).then(done);
  });
